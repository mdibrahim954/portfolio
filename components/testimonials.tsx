  ...
  return (
    <div className={`${gradiantSection}`}>
      <div className={`block ${container} `}>
        ...
        <Carousel
          className={`flex flex-wrap justify-center gap-[20px] mt-[30px]`}
        >
          <CarouselContent>
            {services.map((service, index) => (
              <CarouselItem key={index} className={`basis-1 md:basis-1/3`}>
                <Card
                  className={cn(
                    cardStyle,
                    ` gap-1 border-[0px] p-0 px-[10px] py-[10px] h-full relative`
                  )}
                >
                  <div
                    className={`absolute bg-gradient-to-r from-[#1e40af] to-[#60a5fa] top-0 right-0 px-[10px] py-[5px]  border-l-[2px] border-b-[2px] border-[#60a5fa] rounded-bl-[25px] text-white text-sm  `}
                  >
                    <FontAwesomeIcon
